!function(d3) {
  ...
  function mpld3_Axes(fig, props) {
    ...
    function build_scale(scale, domain, range) {
      var dom = scale === "date" ? d3.time.scale() : scale === "log" ? d3.scale.log() : d3.scaleLinear();
  ...
  mpld3_Axes.prototype.draw = function() {
    ...
    this.zoom1 = d3.zoom();
    this.zoom1.last_t = zoom1.transform.translate();
    this.zoom1.last_s = this.zoom1.scale();
  ...
  mpld3_Axes.prototype.enable_zoom = function() {
    if (this.props.zoomable) {
      this.zoom1.on("zoom", this.zoomed.bind(this, true));
  ...
  mpld3_Axes.prototype.zoomed = function(propagate) {
    ...
    if (propagate) {
      var dt0 = this.zoom1.transform.translate()[0] - this.zoom1.last_t[0];
      var dt1 = this.zoom1.transform.translate()[1] - this.zoom1.last_t[1];
      var ds = this.zoom1.scale() / this.zoom1.last_s;
      this.zoom_x.translate([ this.zoom_x.transform.translate()[0] + dt0, 0 ]);
      this.zoom_x.scale(this.zoom_x.scale() * ds);
      this.zoom_y.translate([ 0, this.zoom_y.transform.translate()[1] + dt1 ]);
      this.zoom_y.scale(this.zoom_y.scale() * ds);
      this.zoom1.last_t = this.zoom1.transform.translate();
      this.zoom1.last_s = this.zoom1.scale();
      this.sharex.forEach(function(ax) {
        ax.zoom_x.translate(this.zoom_x.transform.translate()).scale(this.zoom_x.scale());
      }.bind(this));
      this.sharey.forEach(function(ax) {
        ax.zoom_y.translate(this.zoom_y.transform.translate()).scale(this.zoom_y.scale());
      }.bind(this));
      this.sharex.forEach(function(ax) {
        ax.zoomed(false);
      });
  ...
  mpld3_Axes.prototype.set_axlim = function(xlim, ylim, duration, propagate) {
    ...
    this.zoom1.scale(1).translate([ 0, 0 ]);
    this.zoom1.last_t = this.zoom1.transform.translate();
    this.zoom1.last_s = this.zoom1.scale();
